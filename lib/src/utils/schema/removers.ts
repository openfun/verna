...
function removeWidget(verna: VernaContextProps, id: string) {
  ...
  if (verna.selector) {
    ...
    if (newSchema.required) {
      newSchema.required = newSchema.required?.filter((e: string) => e !== currentField);
    }
  } else {
    ...
    if (newSchema.properties?.[currentSection].required) {
      newSchema.properties[currentSection].required = newSchema.properties[
        currentSection
      ].required?.filter((e: string) => e !== currentField);
    }
  }
